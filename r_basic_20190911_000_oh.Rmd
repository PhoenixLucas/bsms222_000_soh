...
Here is a very simple example showing the general structure of an if-else statement. The basic idea is to print the reciprocal of a unless `a` unless `a` is 0:
...
```{r}
data.frame("a" = a, "is_a_positive" = c(a > 0), "answer1" = 1/a, "answer2" = NA, "result" = ifelse(a >0, 1/a, NA))
```
...
sum(is.na(no_nas))
```
Using this, you can easily solve a previous exercise from chapter 3 to calculate mean:
...
In general, functions are objects, so we assign them to variable names with `<-`. The fuction `function` tells R you are about to define a function. The general form of a function definition looks like this:
...
} #argument도 마음대로 설정할 수 있구나 :)
```

```{r}
avg(a)
avg(a, arithmetic = FALSE) #arithmetic이 FALSE라는 것은 geometric average로 계산하겠다는 것.
```
