...
```{r, echo=FALSE}
knitr::include_graphics(file.path("./rstudio-environment.png"))
```
...
We already use the `install.packages`, `library`, and `ls` functions. We also used the function `sqrt` to solve the quadratic equation above. There are many more prebuilt functions and even more can be added through packages. These functions do not appear in the workspace becasue you did not define them, but they are available for immediate use.
...
This shows you the object name for these datasets. These datsets are objects that can be used by simply typing the name. For example, if you type:

```{r, eval=FALSE}
co2
```

R will show you Mauna Loa atmosphere CO2 concentration data.
...
For more advice, we highly recommend sutying [Hadley Wickham's style guide](http://adv-r.had.co.nz/Style.html).
...
1\. What is the sume of the first 100 postive integers? The formula for the sum of integers $1$ through $n$ is $n(n+1)/2$. Define $n = 100$ and then use R to compute the sum of $1$ through $100$ using the formula. What is the sum?
...
This tells us much more about the object. We see that the table has 51 rows (50 states plus DC) and five variables. We can show the first six lines using this function `head`:

```{r}
head(murders)
```

In this dataset, each state is considered an observation and five variables are repoted for each state.
...
It is important to know that the order of the entries in `murders$poopulation` preserves the order of the rows in our data table.

**Tip**: R comes with a very nice auto-complete functionality that saves us the trouble oftyping out all the names. Try typing `murders$p` then hitting the _tab_ key on your keyboard. This functionality and many other useful auto-complete features are available when working in RStudio.
...
Data frames are a special case of _lists_. We will cover lists in more detail later, but know that they are useful because you can store any combination of different types. BGelow is an example of a list we created for you:
...
You might also encoutner lists without variable names:
...
Matrices are another type of object that are common in R. Matrices are similar to data frames in that they are two-dimensional: they have rows and columns. However, like numeric, character and logical vectors, entries in matrices have to be all the same type. Forthis reason data frames are much more useful for storing dta, since we can have characters, factors and numbers in them.
...
class(murders$region)
```
With one line of code, use the function `level` and `length` to determine the number of regions defined by this dataset.
