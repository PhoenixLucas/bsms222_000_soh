...
그 외에 좋은 참고할만한 사이트.</br>
- [_purrr 패키지를 이용한 함수형프로그래밍이란 무엇인가_](https://statkclee.github.io/parallel-r/ds-fp-purrr.html){target="_blank"}</br>
- [_Cheat sheets for purrr package_](https://github.com/rstudio/cheatsheets/blob/master/purrr.pdf){target="_blank"}</br>
- 이광춘 교수님의 [_xwMooc_](https://statkclee.github.io/xwMOOC/){target="_blank"}. </br>
- [_Data Carpentry for Biologists_](https://datacarpentry.org/semester-biology/){target="_blank"} as taught at University of Florida in Fall 2015. </br>
- [_Data Carpentry Genomics Curriculum_.](https://datacarpentry.org/lessons/#genomics-workshop){target="_blank"}</br>
...
> 정해진 output이 아니면 마음대로 coercion하지 않고 warning을 출력한다는 점에서 debugging에 용이할 수 있다.
...
6    21
```
The `purrr` package provides much more functionality not covered here. For more details you can consult [this online resource.](https://jennybc.github.io/purrr-tutorial/){target="_blank"}

## 5.14 Tidyverse conditionals

A typical data analysis will often involve one or more conditional operation. In Section 4.1 we described the `ifelse` function, which we will use extensively in this book. In this section we present two __dplyr__ functions that provide further functionality for performing conditional operations.

### 5.14.1 `case_when`

The `case_when` function is useful for vectorizing conditional statements. It is similar to `ifelse` but can output any number of values, as opposed to just `TRUE` or `FALSE`. Here is an example splitting number into negative, positives and 0:
...
A common use for this function is to define categorical variables based on existing variables. For example, supposed we want to compare the murder rates in three groups of states: _New England_, _West Coast_, _South_ and _other_. For each state, we need to ask if it is in New England, if it is not we ask if it is in the West Coast, if not we ask if it is in the South (sequentially), and if not we assign _other_. Here is how we use `case_when` to do this:

```{r, echo = FALSE}
data(murders)
murders %>% 
  #group이라는 column을 하나 새로 만들고, 주에 따라 group에 나눠준다.
  mutate(group = case_when(
    abb %in% c("ME", "NH", "VT", "MA", "RI", "CT") ~ "New England",
    abb %in% c("WA", "OR", "CA") ~ "West Coast",
    region == "South" ~ "South",
    TRUE ~ "other")) %>% 
  #group에 의해 grouping 해준다.
  group_by(group) %>% 
...
`murders` is in tidy format (variables are in columns and each row represents observation to that variable) and is stored in a data frame.
...
```
4. Write tidyverse code that is equivalent to this code: `exp(mean(log(murders$population)))`. Write it using the pipe so that each function is called without arguments. Use the dot operator to access the population.

```{r}
murders %>% .$population %>% log() %>% mean() %>% exp()
```

5. Use the `map_df` to create a data frame with three columns named `n`, `s_n`, and `s_n_2`. The first column should contain the numbers 1 through 100. The second and third columns should each contain the sum of 1 through `n` with `n` as the row number.

Function을 만들고, 그 function을 element마다 하나하나 씩 실행해서 원하는 형태의 data frame으로 만들어주는게 map_df이다.
...
`Argument 1 must have names` 에러메시지는 `map_df`에서 쓰이는 함수는 반드시 name을 갖는 list나 data frame을 반환해야 하기 때문에 발생한다 (list도 일종의 간단한 data frame으로 볼 수 있다). `map_df`는 이렇게 생긴 data frame들을 합해서 하나의 큰 data frame을 만들어준다. 그냥 f를 실행하면 numeric vector가 생성되기 때문에 안된다.
따라서 f 자체에서 df가 만들어지도록 `tibble()`을 이용한다.

```{r}

#1부터 100 까지의 integer vector가 있다.
n <- 1:100

#이제 각각의 값을 구해서dataframe으로 만들어준다.
#s_n 구하는 function 만들기
...
different_sums <- mutate(n, s_n = map_df(n, compute_s_n), s_n_2 = map_df(n, compute_s_n_2))
different_sums
...
df1 <- map_df(n, compute_s_n)
df2 <- map_df(n, compute_s_n_2)
df1
df2
```
